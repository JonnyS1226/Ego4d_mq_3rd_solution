...
class PtTransformer(nn.Module):
    ...
    def forward(self, video_list, ensemble=False):

        # batch the video list into feats (B, C, T) and masks (B, 1, T)
        batched_inputs, batched_masks = self.preprocessing(video_list)      # pad to fixed length
        ...
        out_cls_logits = self.cls_head(fpn_feats, fpn_masks)        # [B, 1, T_i]
        # out_offset: List[B, 2, T_i]
        out_offsets = self.reg_head(fpn_feats, fpn_masks)

        # permute the outputs
        # out_cls: F List[B, #cls, T_i] -> F List[B, T_i, #cls]
        out_cls_logits = [x.permute(0, 2, 1) for x in out_cls_logits]       # [B, t_i, #C]
        # out_offset: F List[B, 2 (xC), T_i] -> F List[B, T_i, 2 (xC)]      
        out_offsets = [x.permute(0, 2, 1) for x in out_offsets]             # [B, t_i, 2]
    ...
    def label_points(self, points, gt_segments, gt_labels):
        ...
        return gt_cls, gt_offset    # [378,1] [378,2] for each item in batch
    ...
    def label_points_single_video(self, concat_points, gt_segment, gt_label):
        ...
        lens = lens[None, :].repeat(num_pts, 1)         # 扩充到num_pts，num_pts=2046
        ...
        if self.train_center_sample == 'radius':
            ...
            center_pts = 0.5 * (gt_segs[:, :, 0] + gt_segs[:, :, 1])        # gt seg center [378, 1]
            ...
            # F T x N
            inside_gt_seg_mask = center_seg.min(-1)[0] > 0      # 判断action是否在这个proposal里
        else:
            # inside an gt action
        ...
        return cls_targets, reg_targets         # 最后返回一堆anchor到seg左右边界的distance，外加这些anchor是否有效的mask（即anchor和边长范围在action的seg里）
    ...
    def losses(
        ...
        # gt_* : B (list) [F T, C]
        # fpn_masks -> (B, FT)
        valid_mask = torch.cat(fpn_masks, dim=1)        # [16, 378]

        # 1. classification loss
        # stack the list -> (B, FT) -> (# Valid, )
        gt_cls = torch.stack(gt_cls_labels)             # [b, 378,1]
        pos_mask = torch.logical_and((gt_cls.sum(-1) > 0), valid_mask)      # [b,378]

        # cat the predicted offsets -> (B, FT, 2 (xC)) -> # (#Pos, 2 (xC))
        pred_offsets = torch.cat(out_offsets, dim=1)[pos_mask]              # [67,2]
        gt_offsets = torch.stack(gt_offsets)[pos_mask]                      # [67,2]
        ...
        gt_target = gt_cls[valid_mask]          # [16*67, 1]
    ...
    def inference_single_video(
        ...
        for cls_i, offsets_i, pts_i, mask_i in zip(
            ...
            pred_prob = (cls_i.sigmoid() * mask_i.unsqueeze(-1)).flatten()      # [192]
