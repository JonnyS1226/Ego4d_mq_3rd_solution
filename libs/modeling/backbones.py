...
class ConvTransformerBackbone(nn.Module):
    ...
    def __init__(
        ...
        import json
        f = open('configs/xlnet_config.json')
        js = json.load(f)
        xlnet_config = XLNetConfig.from_dict(js)
        self.xlnet = XLNetModel(xlnet_config)
        # self.downsample_lc = nn.Linear(1024, 512)
        self.downsample_conv = MaskedConv1D(512, 512, kernel_size=3, stride=2, padding=3 // 2, groups=512, bias=False)
    ...
    def forward(self, x, mask):
        ...
        for idx in range(len(self.branch)):
            ...
            #     # =================== xlnet ==========================#
                ...
                x = self.xlnet(inputs_embeds=x, attention_mask=mask.squeeze(1).long())[0]
                x = x.permute(0,2,1)
                # x, mask = self.downsample_conv(x, mask)
                # =================== xlnet ==========================#
            # else:
