...
class Ego4dDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #     feats = np.load(filename).astype(np.float32)                        # [n,d]，n不固定，是 duration * fps / stride, stride=16
        # Get video features
        assert len(self.feature_path) > 0
        if len(self.feature_path) > 1:
            ft_list = []
            for f_p in self.feature_path:
                tmp = torch.load(os.path.join(f_p, video_name + '.pt')) 
                ft_list.append(tmp)
            v_data = torch.cat(ft_list, dim=-1)
        else:
            v_data = torch.load(os.path.join(self.feature_path[0], video_name + '.pt'))
